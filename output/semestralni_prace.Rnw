...
colnames(tableMatrix) <- c("Nové případy", "Nové případy smoothed", "Nové případy na milion", "Nové případy na milion smoothed", "Hospitalizovaní pacienti", "Hospitalizovaní pacienti na milion")
rownames(tableMatrix)<-c("prumer", "modus","median","max","min","skewness","kurtosis","deviation","interquartile")
